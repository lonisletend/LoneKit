...
function createEntry(source = '', target = '') {
  ...
  if (normalizedSource) {
    return {
      ...
      target: encode(normalizedSource),
    };
  }
  if (normalizedTarget) {
    return {
      id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      source: decode(normalizedTarget),
...
function handleSourceInput(index, value) {
  ...
  entries.value[index].target = nextSource ? encode(nextSource) : '';
  ensureEntries();
}
...
function handleTargetInput(index, value) {
  ...
  entries.value[index].source = nextTarget ? decode(nextTarget) : '';
  ensureEntries();
}
...
function applyBatchSource(text) {
  ...
  batchTargetText.value = lines.map(line => encode(line)).join('\n');
}
...
function applyBatchTarget(text) {
  ...
  resetEntries(lines.map(line => createEntry('', line)));
  batchSourceText.value = lines.map(line => decode(line)).join('\n');
}
  ...
  if (text) {
    ...
    if (type === 1) {
      ...
      if (!hasContent(entries.value[lastIndex])) {
        ...
        entries.value[lastIndex].target = encode(text);
      } else {
      ...
      if (!hasContent(entries.value[lastIndex])) {
        ...
        entries.value[lastIndex].source = decode(text);
      } else {
        entries.value.push(createEntry('', text));
      }
    }
