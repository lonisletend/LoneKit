...
async function processImageFile(imageFile) {
  ...
  try {
    ...
    notify('error', t('tool.reader.qrFailed'));
  } finally {
...
<template>
  <div class="qr-reader-root h-full flex flex-col overflow-hidden" :class="{ 'is-dark': resolvedTheme === 'dark' }">
    <SplitPanel>
      <template #left>
