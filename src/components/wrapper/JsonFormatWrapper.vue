...
const cachedComponents = ref(['JsonFormatTool-1']);
...
const currentComponent = computed(() => {
  ...
  const componentName = `JsonFormatTool-${currentTab.id}`;
  ...
  if (!componentCache[componentName]) {
    componentCache[componentName] = defineComponent({
      name: componentName,
      setup() {
        return () => h(JsonFormatTool, { id: currentTab.id });
      }
    });
  }
...
function handleAdd() {
  ...
  // 将新组件添加到缓存列表
  const componentName = `JsonFormatTool-${newId}`;
...
function handleClose(name) {
  ...
  const componentName = `JsonFormatTool-${closedTab.id}`;
