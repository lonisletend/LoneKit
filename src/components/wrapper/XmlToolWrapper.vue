...
const cachedComponents = ref(['XmlTool-1']);
...
const currentComponent = computed(() => {
  ...
  const componentName = `XmlTool-${currentTab.id}`;
  ...
  if (!componentCache[componentName]) {
    componentCache[componentName] = defineComponent({
      name: componentName,
      setup() {
        return () => h(XmlTool, { id: currentTab.id });
      }
    });
  }
...
function handleAdd() {
  ...
  // 将新组件添加到缓存列表
  const componentName = `XmlTool-${newId}`;
...
function handleClose(name) {
  ...
  const componentName = `XmlTool-${closedTab.id}`;
